...
public class jdbcConnection {
    ...
    public static final String SERVER = ":@192.168.0.18:1978";
     ...
     * @throws GwtException
     */
    public String[][] getOnlineSessions(CloudConnection connection) throws GwtException {
    ...
    public String[][] getServerList(Connection connection) throws GwtException {
        ...
        try {
            ...
            // rows.add(new String[] {"服务器", "类型", "状态", "系统", "JAVA", "处理器",
            // "核心", "内存", "磁盘", "系统时间"});
    ...
    public String[][] getMemorySize(Connection connection) throws GwtException {
        ...
        try {
            ...
            for (String[] record : records) {
                Double phyical = Double.parseDouble(record[3]) / (1024 * 1024 * 1024);
                record[3] = numberS2Format.format(phyical);
                Double config = Double.parseDouble(record[4]) / (1024 * 1024 * 1024);
                record[4] = numberS2Format.format(config);
                Double commit = Double.parseDouble(record[5]) / (1024 * 1024 * 1024);
                record[5] = numberS2Format.format(commit);
                Double current = Double.parseDouble(record[6]) / (1024 * 1024 * 1024);
                record[6] = numberS2Format.format(current);
                Double swap = Double.parseDouble(record[7]) / (1024 * 1024 * 1024);
        ...
        if (connection != null) {
            try {
                ...
                for (int i = 0; i < theSQLs.length; i++) {
                    ...
                    String process = "Query " + info[0] + " runs at " + timeFormat.format(new Date()) + ": " + query;
                    ...
                    process = "Query " + info[0] + " done in " + numberS2Format.format((System.currentTimeMillis() - time0) / 1000.0) + " secs: " + query;
                    if (process.length() > 180) {
                        process = process.substring(0, 180) + " ...";
                    }
